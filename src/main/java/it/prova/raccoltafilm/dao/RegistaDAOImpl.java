...
public class RegistaDAOImpl implements RegistaDAO {
	...
	public Optional<Regista> findOneEager(Long id) throws Exception {
		return entityManager.createQuery("from Regista r left join fetch r.films where r.id=:idReg", Regista.class)
				.setParameter("idReg", id).getResultList().stream().findFirst();
	}
