...
public class RegistaServiceImpl implements RegistaService {
	...
	public void rimuovi(Long id) throws Exception {
		...
		try {
			...
			Regista registaInstance=registaDAO.findOneEager(id).orElse(null);
			
			if(registaInstance==null)
				throw new ElementNotFoundException("Regista con id: " + id + " non trovato.");

			if (!registaInstance.getFilms().isEmpty())
				throw new RegistaAssociatoAFilmException("Il regista è associato a " + registaInstance.getFilms().size()
						+ " film, rimuovi i film prima di proseguire");
